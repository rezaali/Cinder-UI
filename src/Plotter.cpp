...
Plotter::Plotter( std::string name, float *value, glm::vec2 range, int bufferSize ) : ControlWithLabel(), mBufferSize(bufferSize), mValue(value), mRange(range), mLineWidth(1.0f), mUpdateAlways(false)
{
    setName( name );
    setDrawBack( true );
    setDrawFill( true );
...
void Plotter::drawFill( std::vector<RenderData> &data, const ci::ColorA &color )
{
    ...
    for(int i = 0; i < limit; i++)
    {
        ...
        vec2 prev = vec2( lmap( (float)prevIndex, 0.0f, (float)limit, _w, w ),
                         lmap<float>( mValues[prevIndex], mRange.x, mRange.y, h, _h ) ) + offset;
        
        vec2 next = vec2( lmap( (float)nextIndex, 0.0f, (float)limit, _w, w ),
                         lmap<float>( mValues[nextIndex], mRange.x, mRange.y, h, _h ) ) + offset;
