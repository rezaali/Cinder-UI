...
TextInput::TextInput( std::string name, const Format &format ) : ControlWithLabel(), mFormat(format), mCallbackFn(nullptr), mClicked(false), mCursorPosition(0), mStartIndex(0)
{
    setName( name );
...
void TextInput::deleteCharacter( )
{
    ...
    if( length )
    {
        ...
        int index = mStartIndex + max( charIndex, 0 );
        if( length > mMaxDisplayLength )
        {
