...
template<typename T>
RangeT<T>::RangeT( string name, T valueLow, T valueHigh, T min, T max, Format format ) : ControlWithLabel(), mUseLowRef(false), mUseHighRef(false), mValueLowRef(new T(valueLow)), mValueHighRef(new T(valueHigh)), mMin(min), mMax(max), mHitHigh(false), mHitCenter(false), mHitLow(false), mCallbackFn(nullptr), mFormat(format), mStickyEnabled(false)
{
    setName( name );
...
template<typename T>
RangeT<T>::RangeT( std::string name, T *valueLow, T *valueHigh, T min, T max, Format format ) : ControlWithLabel(), mUseLowRef(true), mUseHighRef(true), mValueLowRef(valueLow), mValueHighRef(valueHigh), mMin(min), mMax(max), mHitHigh(false), mHitCenter(false), mHitLow(false), mCallbackFn(nullptr), mFormat(format), mStickyEnabled(false)
{
    setName( name );
    setValue( *valueLow, *valueHigh );
}
...
{
    return max( min( mValueLow, 1.0 ), 0.0 );
}

template<typename T>
float RangeT<T>::getNormalizedValueHigh()
{
    return max( min( mValueHigh, 1.0 ), 0.0 );
}
...
template<typename T>
void RangeT<T>::setCallback( const std::function<void(T, T)> &callback )
{
...
void RangeT<T>::input( const glm::vec2 &pt )
{
    ...
    mValueLow = max( min( mValueLow, 1.0 ), 0.0 );
    mValueHigh = max( min( mValueHigh, 1.0 ), 0.0 );    
