...
XYPad::XYPad( string name, vec2 value, const Format &format ) : ControlWithLabel(), mUseRef(false), mValueRef(new vec2(value)), mCallbackFn(nullptr), mFormat(format), mStickyEnabled(false)
{
    setName( name );
    setValue( value );
}

XYPad::XYPad( string name, vec2 *value, const Format &format ) : ControlWithLabel(), mUseRef(true), mValueRef(value), mCallbackFn(nullptr), mFormat(format), mStickyEnabled(false)
{
    setName( name );
    setValue( *value ); 
}
...
{
    return vec2( min( max( mValue.x, 0.0f ), 1.0f ), min( max( mValue.y, 0.0f ), 1.0f ) );
}
...
void XYPad::setCallback( const std::function<void(vec2)> &callback )
{
...
void XYPad::drawOutline( std::vector<RenderData> &data, const ci::ColorA &color )
{
    ...
    addLine( data, color, mHitRect.getUpperLeft() + vec2( 0.0f, value.y*mHitRect.getHeight() ),
            mHitRect.getUpperLeft() + vec2( mHitRect.getWidth(), value.y*mHitRect.getHeight() ) );
