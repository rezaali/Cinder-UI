...
void MultiSlider::setup()
{
    ...
    vec2 size = getSize();
    size.y = mSliderHeight = max( size.y - mPadding.mTop - mPadding.mBottom,
...
{
    return max( min( mDataMap[key]->mValue, 1.0 ), 0.0 );
}
...
void MultiSlider::setCallback( const std::function<void(Data)> &callback )
{
