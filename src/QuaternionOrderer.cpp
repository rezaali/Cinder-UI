...
QuaternionOrderer::QuaternionOrderer( std::string name, const Format &format ) : ControlWithLabel(), mFormat( format ), mEasingFn( easeInOutSine )
{
    setName( name );
    setDrawBack( true );
    setDrawFill( true );
...
void QuaternionOrderer::addQuat( float time, ci::quat quat )
{
    int index = 0;
    for( auto& it : mTimes ) {
        if( time == it )
