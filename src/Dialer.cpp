...
template<typename T>
DialerT<T>::DialerT( std::string name, T value, T min, T max, Format format ) : ControlWithLabel(), mUseRef(false), mValueRef(new T(value)), mMin(min), mMax(max), mFormat(format), mCallbackFn(nullptr)
{
    setName( name );
    setValue( value );
}
...
template<typename T>
DialerT<T>::DialerT( std::string name, T *value, T min, T max, Format format ) : ControlWithLabel(), mUseRef(true), mValueRef(value), mMin(min), mMax(max), mFormat(format), mCallbackFn(nullptr)
{
    setName( name );
    setValue( *value );
}
...
Rectf DialerT<T>::getHitRect()
{
    ...
    int index = min( max( (float)mCurrentIndex, 0.0f ), (float)mNumOfPrecisionZones - 1.0f );
