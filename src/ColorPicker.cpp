...
ColorPicker::ColorPicker( std::string name, const ColorA &color, const Format &format ) : ControlWithLabel(), mColor(color), mFormat(format), mCallbackFn(nullptr), mHitPoint(vec2(0.0)), mUseRef(false), mColorRef(new ColorA(color))
{
    setName( name );
...
ColorPicker::ColorPicker( std::string name, ci::ColorA *color, const Format &format ) : ControlWithLabel(), mColor(*color), mFormat(format), mCallbackFn(nullptr), mHitPoint(vec2(0.0)), mUseRef(true), mColorRef(color)
...
void ColorPicker::load( const ci::JsonTree &data )
{
    ...
    {
        setColor( ColorA( data.getValueForKey<float>( "RED" ), data.getValueForKey<float>( "GREEN" ),
                         data.getValueForKey<float>( "BLUE" ), data.getValueForKey<float>( "ALPHA" ) ) );
        trigger(); 
    }
    View::load( data );
}
...
void ColorPicker::setCallback( const std::function<void(ColorA)> &callback )
{
...
void ColorPicker::mouseWheel( ci::app::MouseEvent &event )
{
    if( isHit( event.getPos() ) && !mHit )
    {
        ...
        mColor.a = max( min( mColor.a, mFormat.mAlphaLimits.y ), mFormat.mAlphaLimits.x );
        updateColorRef();
        updateLabel();
        setNeedsDisplay();
