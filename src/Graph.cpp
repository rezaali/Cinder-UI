...
Graph::Graph( std::string name, int bufferSize, float *xValues, float *yValues, glm::vec2 xRange, glm::vec2 yRange ) : Control(), mBufferSize(bufferSize), mXValues(xValues), mYValues(yValues), mXRange(xRange), mYRange(yRange), mLineWidth(1.0f), mClosed(true)
{
    setName( name );
    setDrawBack( true );
    setDrawFill( true );
...
void Graph::drawFill( std::vector<RenderData> &data, const ci::ColorA &color )
{
    ...
    for(int i = 0; i < limit; i++)
    {
        ...
        vec2 prev = vec2( lmap<float>( mXValues[prevIndex], mXRange.x, mXRange.y, _w, w ),
                         lmap<float>( mYValues[prevIndex], mYRange.x, mYRange.y, _h, h ) ) + offset;
        
        vec2 next = vec2( lmap<float>( mXValues[nextIndex], mXRange.x, mXRange.y, _w, w ),
                         lmap<float>( mYValues[nextIndex], mYRange.x, mYRange.y, _h, h ) ) + offset;
